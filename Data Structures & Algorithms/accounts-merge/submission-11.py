class Solution:
    def accountsMerge(self, accounts: List[List[str]]) -> List[List[str]]:
            ...
            #Create our INDEPENDENT union trackers (One for tracking root of account indexs, one for height of tree)
            # parentAccounts: The array index represents the same accountIndex as from accountse array. The value is the root account index of that accountIndex
            # So for ex: root of parentAccounts[0] = 3 would mean accounts[0] and accounts[3] share same root and are grouped/unioned
            parentAccounts = list(range(len(accounts)))
            rank = [1] * len(accounts) # This is for optimization, the approximate height of tree rooted at each account index
            ...
            def union(currAccountIndex, existingAccountIndex):
                ...
                #Group by size of accounts to have a balanced tree
