...
class MyCalendar:
    def __init__(self):
        self.root = Node(0, 1_000_000_000 - 1)

    def book(self, startTime: int, endTime: int) -> bool:
        if not self.query(self.root, startTime, endTime - 1):
            return False
        self.insert(self.root, startTime, endTime - 1)
        return True
    ...
    def query(self, node, startTime, endTime) -> bool:
        ...
        if endTime <= mid:
            ...
            return (self.query(node.leftChild, startTime, mid) and
                    self.query(node.rightChild, mid + 1, endTime))
    ...
    def insert(self, node, startTime, endTime):
        ...
        if node.rightChild is None:
            node.rightChild = Node(mid + 1, node.end)
        if endTime <= mid:
            ...
            self.insert(node.rightChild, mid + 1, endTime)
