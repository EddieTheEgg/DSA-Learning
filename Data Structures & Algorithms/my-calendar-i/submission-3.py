...
class MyCalendar:
    ...
    def book(self, startTime: int, endTime: int) -> bool:
        # Check first — don't mutate anything
    ...
    def query(self, node, startTime, endTime) -> bool:
        ...
        if startTime < mid:
            left_ok = self.query(node.leftChild, startTime, min(endTime, mid))
            if not left_ok:
                return False

        if endTime > mid:
            right_ok = self.query(node.rightChild, max(startTime, mid), endTime)
            if not right_ok:
                return False
