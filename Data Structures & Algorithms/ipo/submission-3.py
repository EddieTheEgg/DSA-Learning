...
class Solution:
    def findMaximizedCapital(self, k: int, w: int, profits: List[int], capital: List[int]) -> int:
        ...
        for _ in range(k):
            ...
            # This is saying if we have no avaiable projects to work on. This can only happen
            # when projects left in minCapital/minHeap is too expensive or has more capital requirement than
            # the capital (w) that we have currently. With no capital projects to pop into our maxHeap, we cannot pop a null root for maxHeap
            if not maxProfit:
                break

            #After we filtered out all available projects we can work on from minCapital by moving
            # them to maxProfit, we can pop the root of maxProfit since it is a maxHeap
            # to get the max profit which adds to our capital (w).
            # Each root pop of maxHeap counts as a project we've worked on, and guaranteed to give us the highest profit
            # in that iteration since each project iteration k, we loop through minCapital (which is sorted low to high)
            # to fetch all possible projects that satisfies our current capital (w).
